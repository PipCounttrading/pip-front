...
const Deposit = () => {
  ...
  const [code, setCode] = useState("1LzwPxruykDnrDnLG93Uv1XTSz822rD4XZ");
  ...
  const onCoinChange = (e) => {
    ...
    if (value == "btc") {
      setBarcode("btc");
      setCode("1LzwPxruykDnrDnLG93Uv1XTSz822rD4XZ");
    } else if (value == "eth") {
      setBarcode("eth");
      setCode("0x6c1c214c51777a364efb7dddd49c649511695be7");
    // } else if (value == "shib") {
    //   setBarcode("shib");
  ...
  useEffect(() => {
    ...
    fetch("https://lucky-cape-fox.cyclic.app/deposit", {
      signal: signal,
  ...
  const onDeposit = async () => {
    ...
    const res = await fetch("https://lucky-cape-fox.cyclic.app/deposit", {
      method: "POST",
  ...
  const showModal = () => {
    ...
    if (parseFloat(amt) >= 100) {
      // Open the modal or perform any desired action
      setIsModalOpen(true);
    } else {
      // Don't open the modal
      setIsModalOpen(false);
      notify("Amount is less than 100, cannot continue deposit");
    }
  };
  const handleOk = () => {
    setIsModalOpen(false);
            ...
            <section class="bg-[#f5f6fa] md:flex md:mt-10 flex-col justify-center h-screen">
              <div class="flex flex-col items-center font-semibold text-2xl px-6 pt-8 h-full">
                ...
                >
                  ...
                  <option value="usdte">USDT</option>
                </select>
